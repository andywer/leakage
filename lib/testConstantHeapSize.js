...
const heapGrowthSensitivity = 1024  // 1kB

module.exports = assertConstantHeapSize

function assertConstantHeapSize (heapDiffs, { iterations, gcollections }) {
...
function getSubsequentHeapGrowths (heapDiffs) {
  ...
  heapDiffs.forEach(heapDiff => {
    if (heapDiff.change.size_bytes > heapGrowthSensitivity) {
